...
if ($paziente_id_aukera) {
    ...
    if ($stm_egiaztatu->fetch()) {
        $stmtN = $pdo->prepare("SELECT erregistro_data, glukosa_mg_dl, tentsio_sistolikoa, tentsio_diastolikoa, pisua_kg, pultsua_ppm, sintomak FROM Neurketak WHERE paziente_id = ? ORDER BY erregistro_data DESC");
